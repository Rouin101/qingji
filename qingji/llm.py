...
def build_claim_evidence_review_prompt(
    ...
    *,
    ...
    prompt = (
        ...
        "已确认授权的证据卡，是否能凭卡片本身支持当前结论的核心事实、直接反驳该核心事实，"
        "或只能作为背景。若证据明确记录了相同的对象、行为或状态和方向，即使结论额外使用了"
        "‘多数’‘普遍’‘显著’或因果措辞，也应对核心事实返回 support；范围、强度、数量和因果"
        "是否过界由本地规则降级为‘部分支持’，不要在这里把核心事实一并判为 context。具体数字、"
        "肯定/否定方向、对象、时间或场景明显不一致时仍不能返回 support。只有卡片明确给出相反"
        "内容时，才返回 contradict。不同人的体验可同时成立，不得以另一人的顺利体验反驳"
        "‘一名受访者遇到困难’；但结论使用‘普遍’‘全部’等群体或绝对范围时，明确的群体级"
        "相反记录可以返回 contradict，单个不同个例仍只作为 context。共享主题、词语或领域"
...
def build_material_claim_candidate_prompt(
    ...
    *,
    ...
    prompt = (
        ...
        "数据罗列和纯客观事实。每条候选只能表达一个可核验判断，不得用分号串联多个发现，"
        "也不要把‘材料支持什么、不能支持什么’写进同一条候选。若没有合格的主观发现，"
        "返回空数组。只返回一个 JSON 对象，"
        "不要 Markdown 代码围栏。\n\n"
        "JSON 字段必须为：candidates（数组，每项包含 claim_text、segment_ids；"
        f"最多 {max_candidates} 项；segment_ids 至少一个且只能使用给定 segment_id 的整数）、"
        "uncertainties（最多 8 条）。claim_text 不超过 120 字，且必须由所列片段直接复核。\n\n"
        "材料片段（每行一个 JSON 对象）：\n"
        f"{material_context}"
    )
    return prompt, allowed_ids
...
def build_evidence_card_generation_prompt(
    ...
    *,
    ...
    prompt = (
        ...
        "evidence_type 按卡片原文实际记录的内容选择，而不是机械沿用上传者角色：受访者"
        "原话用 interview_statement，工作人员原话用 staff_explanation，现场计数或观察用"
        "field_observation，表格、台账或正式文本用 formal_record；只有研究者的解释、归纳"
        "或建议才用 team_analysis。综合报告中嵌入的访谈和观察不能一律标成 team_analysis。"
        "不要为目录、孤立标题、改进建议或文档中专门列出的‘待核验结论’生成证据卡；"
        "待核验结论必须回到它所依据的访谈、观察或正式记录。"
        + card_count_rule
        + "只返回 JSON，"
        "不要 Markdown 围栏。\n\n"
...
def build_evidence_review_prompt(
    ...
    *,
    ...
    prompt = (
        ...
        "作为当前项目的一条可追溯材料。审核的是卡片与原文是否一致，不是它能否单独证明"
        "一个宽泛结论。只要脱敏原文包含可复核的陈述、观察、说明、正式记录或明确标注的"
        "团队分析，且标题和摘要没有增写原文不存在的人物、数字、时间、地点、范围或因果，"
        "应选择 approved；样本有限、仅代表个人、仍需交叉验证等局限写入 uncertainties，"
        "不能仅因这些局限选择 rejected。只有原文为空或仅为目录/孤立标题、标题摘要明显"
        "曲解或虚构、来源类型冒充一手事实等明确卡片质量问题，才选择 rejected。"
        "团队分析可以作为标明类型的背景材料，但不能冒充受访者事实。只返回一个 JSON 对象，"
        "不要 Markdown 围栏。\n\n"
...
def build_evidence_review_batch_prompt(
    ...
    *,
    ...
    prompt = (
        ...
        "是否适合作为当前项目的一条可追溯材料。审核的是卡片与原文是否一致，不是它能否"
        "单独证明宽泛结论。只要原文包含可复核的陈述、观察、说明、正式记录或明确标注的"
        "团队分析，且标题摘要没有扩写，应选择 approved；样本有限、只代表个人、信息尚需"
        "交叉验证等局限写入 uncertainties，不能仅因这些局限选择 rejected。只有原文为空或"
        "仅为目录/孤立标题、标题摘要明显曲解或虚构、来源类型冒充一手事实等明确质量问题，"
        "才选择 rejected。团队分析可作为标明类型的背景材料，但不能冒充受访者事实。"
