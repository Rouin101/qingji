...
def step_import_and_approve(app: AppTest) -> None:
    ...
    app.text_area(key="material_draft_text").set_value(IMPORT_TEXT)
    app.text_input(key="material_filename").set_value(MATERIAL_FILENAME)
    app.selectbox(key="material_source_role").select("受访者")
    app.text_input(key="material_context").set_value("线上便民服务体验访谈")
    app.text_input(key="material_custom_terms").set_value("受访者甲")
    app.radio(key="material_consent_choice").set_value("confirmed")
    app.date_input(key="material_captured_at").set_value(date(2026, 8, 25))
    app.checkbox(key="material_is_fictional").set_value(True)
    app.checkbox(key="material_confirmed").set_value(True)
    app.button(
        key="FormSubmitter:material_import_form-本地检查并生成证据卡"
    ).click()
    app.run()
